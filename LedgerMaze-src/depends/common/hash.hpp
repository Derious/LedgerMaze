...
/* H(R||m) , R is the ECPpoint and m is BIGNUM */
void Hash_ECP_to_BIGNUM(EC_POINT *&R, const BIGNUM *m , BIGNUM *&h)
{
    std::string tmp(EC_POINT_point2hex(group,R,POINT_CONVERSION_UNCOMPRESSED, bn_ctx));

    if (m!=NULL)
    {
        tmp.append(BN_bn2hex(m));
        /* code */
    }
