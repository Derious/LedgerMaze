...
#include "../src/one_out_of_n_protocol.hpp"
...
BOOST_AUTO_TEST_CASE(one_out_of_n)
{
    ...
    One_Out_of_N_Proof_new(proof);

    prover.proof(commits, index, r, true, *proof.proof);

    sigma::SigmaPlusVerifier<secp_primitives::Scalar,secp_primitives::GroupElement> verifier(g, h_gens, n, m);

    BOOST_CHECK(verifier.verify(commits, *proof.proof, true));
